...
import holidays
from dateutil.relativedelta import relativedelta # DEBUG
import traceback
...
def process_context_menu_actions():
    ...
    if 'context_action' in query_params and 'empreendimento' in query_params:
        ...
        if action == 'take_baseline':
            try:
                version_name = take_gantt_baseline(st.session_state.df_data, empreendimento, "Gantt")
        ...
        <!DOCTYPE html>
            ...
            <head>
                ...
                <style>
                    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
                    ...
                    // No setupEventListeners ou initGantt, adicione:
                    document.addEventListener('contextmenu', function(e) {{
                        if (e.target.closest('.gantt-container') || e.target.closest('.gantt-chart-content')) {{
                            e.preventDefault();
                        }}
                    }});
                    .toolbar-btn:hover {{
            ...
            <body>
                ...
                <script>
                    ...
                    // Adicionar esta função ao JavaScript do Gantt
                    # --- CONTEXT MENU INTEGRATION ---
                    function setupGanttContextMenu() {{
                        const ganttContainer = document.getElementById('gantt-container-{project["id"]}');
                        if (!ganttContainer) {{
                            console.log('Gantt container not found, retrying...');
                            setTimeout(setupGanttContextMenu, 1000);
                            return;
                        }}
                        
                        console.log('Setting up context menu for Gantt container');
                        ...
                        ganttContainer.addEventListener('contextmenu', function(e) {{
                            ...
                            menu.innerHTML = `
                                <div style="padding: 8px 15px; cursor: pointer; border-bottom: 1px solid #eee;" 
                                    onclick="window.ganttTakeBaseline()">📸 Criar Linha de Base</div>
                                <div style="padding: 8px 15px; cursor: pointer; color: #999;" 
                                    onclick="this.parentNode.remove()">✕ Fechar</div>
                            `;
                    ...
                    // Global function for baseline creation
                    window.ganttTakeBaseline = function() {{
                        const timestamp = new Date().getTime();
                        const url = `?context_action=take_baseline&empreendimento={selected_empreendimento_baseline}&t=${timestamp}`;
                        console.log('Taking baseline, navigating to:', url);
                        window.location.href = url;
                    }};

                    // Initialize context menu when Gantt is ready
                    setTimeout(setupGanttContextMenu, 2000);
...
with st.spinner("Carregando e processando dados..."):
    ...
    if df_data is not None and not df_data.empty:
         # --- INICIALIZAÇÃO DO SESSION_STATE PARA MENU DE CONTEXTO (ADICIONAR ESTAS LINHAS) ---
        ...
        # --- PROCESSAR AÇÕES DO MENU DE CONTEXTO (ADICIONAR ESTA LINHA) ---
        ...
        # --- SEÇÃO DE BASELINES (MOVIDA PARA FORA DA SIDEBAR) ---
            ...
            def create_gantt_context_menu_component(selected_empreendimento):
                """Componente simples do menu de contexto - apenas para mensagens"""
                ...
                # Componente HTML mínimo apenas para garantir que as funções existam
                context_menu_html = """
                <script>
                // Função global como fallback
                if (!window.ganttTakeBaseline) {
                    window.ganttTakeBaseline = function() {
                        const timestamp = new Date().getTime();
                ...
                st.components.v1.html(context_menu_html, height=0)
        # --- FIM DO NOVO LAYOUT ---
        ...
        with tab1:
            ...
            # ADICIONE ESTE COMPONENTE DE CONTEXTO APENAS UMA VEZ
            if selected_empreendimento_baseline:
